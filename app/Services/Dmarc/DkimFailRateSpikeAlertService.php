...
class DkimFailRateSpikeAlertService
{
    ...
    public function evaluate(DmarcAlertRule $rule, ?Carbon $now = null): ?array
    {
        ...
        $currentRate = $this->failRatePercent($current['dkim_fail_messages'], $current['total_messages']);
        $baselineRate = $this->failRatePercent($baseline['dkim_fail_messages'], $baseline['total_messages']);
        ...
        return [
            ...
            'current_dkim_fail_messages' => $current['dkim_fail_messages'],
            'current_fail_rate' => $currentRate,
            'baseline_total_messages' => $baseline['total_messages'],
            'baseline_dkim_fail_messages' => $baseline['dkim_fail_messages'],
    ...
    /**
     * @return array{total_messages:int,dkim_fail_messages:int}
     */
    private function aggregatePeriod(DmarcAlertRule $rule, Carbon $start, Carbon $end): array
    {
        $totals = DmarcRecord::query()
            ...
            ->selectRaw("COALESCE(SUM(CASE WHEN LOWER(dmarc_records.dkim) = 'fail' THEN dmarc_records.message_count ELSE 0 END), 0) as dkim_fail_messages")
            ->first();

        return [
            'total_messages' => (int) ($totals?->total_messages ?? 0),
            'dkim_fail_messages' => (int) ($totals?->dkim_fail_messages ?? 0),
        ];
    }
