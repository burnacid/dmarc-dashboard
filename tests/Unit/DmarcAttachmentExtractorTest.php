...
class DmarcAttachmentExtractorTest extends TestCase
{
    ...
    public function test_it_extracts_xml_from_zip_entry_without_xml_extension(): void
    {
        if (! class_exists(ZipArchive::class)) {
            $this->markTestSkipped('ZipArchive extension is not available.');
        }

        $xml = '<feedback><report_metadata><report_id>zip-no-ext</report_id></report_metadata></feedback>';
        $tmpFile = tempnam(sys_get_temp_dir(), 'zip_test_');
        $zip = new ZipArchive();
        $zip->open($tmpFile, ZipArchive::CREATE | ZipArchive::OVERWRITE);
        $zip->addFromString('google.com!example.com!12345', $xml);
        $zip->close();
